package com.dushuge.controller.ui.audio.manager;
...
import com.dushuge.controller.BuildConfig;
...
import com.dushuge.controller.eventbus.AudioSoundServiceRefresh;
...
public class Mp3Service extends Service {
    ...
    private void startText(BookChapter currentChapter) {
        ...
        try {
            ...
            // 内容
            String content = instance.getChapterContent().getContent();
            if (content != null && !TextUtils.isEmpty(content)) {
                // 判断字节长度是否限制
                int count = content.length() / 4000;
                if (content.length() % 4000 > 0) {
                    count++;
                }
                if (instance.getChapterContent().getContents() != null) {
                    instance.getChapterContent().getContents().clear();
                }
                List<String> contents = new ArrayList<>();
                for (int i = 0; i < count; i++) {
                    if (i == count -1) {
                        contents.add(i, content.substring(4000 * i));
                        break;
                    }
                    contents.add(i, content.substring(4000 * i, 4000 * (i + 1)));
                }
                instance.getChapterContent().setContents(contents);
            }
            int code = mTts.startSpeaking(instance.getChapterContent().getContents().get(0), mTtsListener);
    ...
    public SynthesizerListener mTtsListener = new SynthesizerListener() {
        ...
        public void onCompleted(SpeechError error) {
            ...
            if (instance.getChapterContent().getContents() != null && !instance.getChapterContent().getContents().isEmpty()) {
                instance.getChapterContent().getContents().remove(0);
                if (!instance.getChapterContent().getContents().isEmpty()) {
                    int code = mTts.startSpeaking(instance.getChapterContent().getContents().get(0), mTtsListener);
                    if (code != ErrorCode.SUCCESS) {
                        setCode(code);
                    }
