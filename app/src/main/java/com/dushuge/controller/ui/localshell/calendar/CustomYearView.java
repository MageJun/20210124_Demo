...
public class CustomYearView extends YearView {
    ...
    public CustomYearView(Context context) {
        super(context);
        mTextPadding = ImageUtil.dp2px(context, 3);

        mLeapYearTextPaint.setTextSize(ImageUtil.dp2px(context, 12));
    ...
    protected void onDrawMonth(Canvas canvas, int year, int month, int x, int y, int width, int height) {
        ...
        if (month == 2 && isLeapYear(year)) {
            ...
            canvas.drawText("闰年",
                    x + mItemWidth / 2 - mTextPadding + w + ImageUtil.dp2px(getContext(), 6),
                    y + mMonthTextBaseLine,
                    mLeapYearTextPaint);
        }
    }
