...
public class ReaderMessageReceiver extends MessageReceiver {
    ...
    public static void sendNotification(final Context context, String title, String summary, String skip_type, String content) {
        ...
        // 设置点击
        Intent intent = null;
        if (!TextUtils.isEmpty(skip_type) && !TextUtils.isEmpty(content)) {
            intent = intentTo(context, Integer.parseInt(skip_type), content, title);
        } else {
            intent = new Intent("action");
        }
        if (intent !=  null) {
            PendingIntent pendingIntents = PendingIntent.getActivity(context,
                    1, intent, PendingIntent.FLAG_ONE_SHOT);
            builder.setContentIntent(pendingIntents);
        }
        mNotificationManager.cancelAll();
        mNotificationManager.notify(BuildConfig.APPLICATION_ID, 180, builder.build());
    }
    ...
    public void onMessage(Context context, CPushMessage cPushMessage) {
        MyToash.Log("HonMessage",cPushMessage.getTitle()+"   "+cPushMessage.getContent());
