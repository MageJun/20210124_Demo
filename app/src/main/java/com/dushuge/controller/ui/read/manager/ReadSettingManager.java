...
import com.dushuge.controller.ui.read.util.SharedPreUtils;
...
public class ReadSettingManager {
    ...
    public int getBrightness(Activity activity) {
        int brightness = sharedPreUtils.getInt(SHARED_READ_BRIGHTNESS, -1);
        if (brightness == -1) {
            // 使用系统默认的亮度
            return BrightnessUtil.getScreenBrightness(activity);
        } else {
            return brightness;
        }
    }
